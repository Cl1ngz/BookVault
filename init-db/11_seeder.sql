...
BEGIN
    ...
    -- D. Gatunki literackie po angielsku (jak w StoryGraph)
    --    Uruchamia się zawsze gdy tabela jest pusta – naprawia też istniejącą bazę
    IF (SELECT count(*) FROM gatunki) = 0 THEN
        ...
        RAISE NOTICE 'Gatunki zostały dodane!';
    ...
    IF (SELECT count(*) FROM ksiazka_gatunek) = 0 AND (SELECT count(*) FROM ksiazki) > 0 THEN
        ...
        RAISE NOTICE 'Gatunki zostały przypisane do książek!';
    END IF;
END $$;
