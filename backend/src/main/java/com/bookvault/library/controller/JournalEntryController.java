...
public class JournalEntryController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateEntry(@PathVariable Integer id,
        ...
        if (body.get("entryDate") != null)
            entry.setEntryDate(LocalDate.parse(body.get("entryDate").toString()));
        if (body.get("cumulativePages") != null)
            entry.setCumulativePages(((Number) body.get("cumulativePages")).intValue());
