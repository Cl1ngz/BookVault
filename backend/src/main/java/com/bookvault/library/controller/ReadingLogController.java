...
public class ReadingLogController {
    ...
    public ResponseEntity<?> getShelf(
        ...
        if (status != null) {
            try {
                ...
                return ResponseEntity.ok(
                        readingLogRepository.findByReader_IdAndStatus(reader.getId(), readingStatus)
                );
    ...
    public ResponseEntity<?> addToShelf(
        ...
        ReadingStatus status = readingLogDto.getStatus() != null
                ? readingLogDto.getStatus()
    ...
    public ResponseEntity<?> updateShelfEntry(
        ...
        if (readingLogDto.getPagesRead() != null) {
            log.setPagesRead(readingLogDto.getPagesRead());
        }

        if (readingLogDto.getStartedAt() != null) {
            log.setStartedAt(readingLogDto.getStartedAt());
        }

        if (readingLogDto.getFinishedAt() != null) {
            log.setFinishedAt(readingLogDto.getFinishedAt());
        }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> removeFromShelf(
            @PathVariable Integer id,
            HttpServletRequest request
    ) {
        Reader reader = authenticate(request);
